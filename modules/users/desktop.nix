...

{
  ...
  isEnabled = userCfg: userCfg.desktop.enable;

  homeConfig = _username: userCfg:
    lib.mkIf userCfg.desktop.enable {
  ...
  assertions = username: userCfg:
    lib.optionals userCfg.desktop.enable [
      {
        assertion = config.alanix.desktop.enable;
        message = "alanix.users.accounts.${username}.desktop.enable requires alanix.desktop.enable = true.";
      }
    ];
}
