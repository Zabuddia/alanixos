...
let
  ...
  backupPrepScript = pkgs.writeShellScript "alanix-jellyfin-cluster-backup-runtime" ''
    set -euo pipefail
    ...
    emit_prep_step ${toString prepStepCount} ${toString prepStepCount} ${lib.escapeShellArg "snapshotting jellyfin sqlite databases"}
  ...
  restoreScript =
    let
      mediaFolderOwnershipFixups = lib.concatMapStringsSep "\n"
        ...
        '')
        (builtins.attrValues cfg.mediaFolders);
      recordingOwnershipFixup =
        lib.optionalString (
          cfg.liveTv.recordingPath != null
          && !(lib.any (folderCfg: folderCfg.path == cfg.liveTv.recordingPath) (builtins.attrValues cfg.mediaFolders))
        ) ''
          chown -R root:root ${lib.escapeShellArg cfg.liveTv.recordingPath}
        '';
    in
    pkgs.writeShellScript "alanix-jellyfin-cluster-restore-runtime" ''
      ...
      chown -R jellyfin:jellyfin ${lib.escapeShellArg cfg.dataDir}
      ${mediaFolderOwnershipFixups}
      ${recordingOwnershipFixup}

      system_xml=${lib.escapeShellArg "${cfg.dataDir}/config/system.xml"}
...
in
{
  config = lib.mkIf enabled (lib.mkMerge [
    ...
    alanix.clusterServices.jellyfin = {
      ...
      targetUnits = [ "jellyfin.service" ];
      exposureUnits = [ "jellyfin.service" ];
      tmpfiles = [
        "d ${cfg.backupDir} 0750 jellyfin ${backupRepoUserGroup} - -"
      ];
      webEndpoints = [
        {
          id = "jellyfin";
    ...
    (helpers.mkActiveTargetUnits [ "jellyfin.service" ])
  ]);
}
